...
import { createMemberProfileModerationFilterSettings } from '@/utils/filterDefaults';
...
const form = reactive(createMemberProfileModerationFilterSettings());

function applySettings(settings) {
    Object.assign(form, createMemberProfileModerationFilterSettings(), settings);
}

async function loadSettings() {
    loading.value = true;
    try {
        applySettings(await apiFetch('/api/filter/member/profile-moderation'));
    } catch {
        toast.add({
            severity: 'error',
            summary: t('filter.toast.loadFailed'),
            detail: t('filter.memberProfileModeration.loadFailed'),
            life: 4000
        });
    } finally {
        loading.value = false;
...
async function saveSettings() {
    ...
    try {
        ...
        toast.add({
            ...
            detail: t('filter.memberProfileModeration.saveSuccess'),
            life: 3000
        });
    } catch {
        toast.add({
            ...
            detail: t('filter.memberProfileModeration.saveFailed'),
            life: 4000
        });
    } finally {
        saving.value = false;
        ...
        <div class="grid grid-cols-12 gap-8">
            <div class="col-span-12 xl:col-span-8">
                <div class="card flex flex-col gap-6">
                    <div>
                        <div class="font-semibold text-xl mb-2">{{ t('filter.memberProfileModeration.title') }}</div>
                        <p class="text-muted-color m-0">
                            {{ t('filter.memberProfileModeration.description') }}
                        </p>
                    </div>
                    ...
                    <div>
                        <div class="font-semibold text-lg mb-1">{{ t('filter.memberProfileModeration.detectionScope') }}</div>
                    </div>

                    <div class="flex items-center gap-3">
                        <Checkbox
                            v-model="form.isFilterAppliedToName"
                            inputId="member-moderation-name"
                            binary
                            :disabled="loading || saving"
                        />
                        <label for="member-moderation-name">{{ t('filter.memberProfileModeration.checkName') }}</label>
                    </div>

                    <div class="flex items-center gap-3">
                        <Checkbox
                            v-model="form.isFilterAppliedToIcon"
                            inputId="member-moderation-icon"
                            binary
                            :disabled="loading || saving"
                        />
                        <label for="member-moderation-icon">{{ t('filter.memberProfileModeration.checkIcon') }}</label>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <Checkbox
                            ...
                            inputId="member-moderation-custom-flag"
                            binary
                            :disabled="loading || saving"
                        />
                        <label for="member-moderation-custom-flag">{{ t('filter.moderation.useCustomFlag') }}</label>
                    </div>
                    ...
                    <div v-if="form.isUseCustomFlag" class="grid grid-cols-12 gap-4">
                        ...
                        >
                            <label :for="`member-moderation-${category.key}`">{{ category.label }}</label>
                            <InputNumber
                                :id="`member-moderation-${category.key}`"
